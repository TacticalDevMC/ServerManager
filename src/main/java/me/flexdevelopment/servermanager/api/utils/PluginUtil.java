...
public class PluginUtil {
    ...
    public static String load(String name) {
        ...
        if (!pluginDir.isDirectory()) {
            return ServerManager.getInstance().getMessageModule().getInvalidDiractory();
        }
            ...
            for (File f : pluginDir.listFiles()) {
                if (f.getName().endsWith(".jar")) {
                    try {
                        ...
                        return ServerManager.getInstance().getMessageModule().getCannotFindLoad();
                    }
                }
            }
        }
        ...
        try {
            ...
            e.printStackTrace();
            return ServerManager.getInstance().getMessageModule().getInvalidPlugin();
        }

        target.onLoad();
        Bukkit.getPluginManager().enablePlugin(target);

        return ServerManager.getInstance().getMessageModule().getPluginLoaded().replace("%plugin%", target.getName());
